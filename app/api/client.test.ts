...
async function runIntegrationTests() {
    console.log(`--- Starting ApiClient Integration Tests against ${API_BASE_URL} ---`);
    ...
    try {
        ...
        console.log(`Get Conversations Result (limit 5): Found ${initialConversations.data.length} conversations.`);
        ...
        const createData: ConversationCreateRequest = { title: `Integration Test Convo ${timestamp}` };
        ...
        console.log(`   [PASS] Conversation created with ID: ${createdConversationId}`);

        // Test 4: Get Conversation Messages (initially empty or contains initial)
        console.log('\n[Test 4] Get Conversation Messages (Initial)');
        if (!createdConversationId) throw new Error("Cannot run Test 4 without createdConversationId");
        const initialMessages = await apiClient.getConversationMessages(createdConversationId, { limit: 10 });
        console.log(`Get Messages Result: Found ${initialMessages.data.length} messages.`);
        ...
        console.log('\n[Test 5] Create Message');
        if (!createdConversationId) throw new Error("Cannot run Test 5 without createdConversationId");
        ...
        const newMessage = await apiClient.createMessage(createdConversationId, messageData);
        ...
        console.log('\n[Test 6] Update Conversation');
        if (!createdConversationId) throw new Error("Cannot run Test 6 without createdConversationId");
        const updatedTitle = `Updated Test Convo ${timestamp}`;
        const updateData: ConversationUpdateRequest = { title: updatedTitle };
        const updatedConversation = await apiClient.updateConversation(createdConversationId, updateData);
        ...
        console.log('\n[Test 7] Get Conversation Messages (After Adding)');
        if (!createdConversationId) throw new Error("Cannot run Test 7 without createdConversationId");
        const messagesAfterAdd = await apiClient.getConversationMessages(createdConversationId, { limit: 10 });
        console.log(`Get Messages Result: Found ${messagesAfterAdd.data.length} messages.`);
        expect(messagesAfterAdd.data.length).toBeGreaterThanOrEqual(1); // Should have at least the message we added
        const addedMsg = messagesAfterAdd.data.find(m => m.message_id === messageData.message_id);
        ...
        console.log('\n[Test 8] Delete Specific Conversation (Cleanup)');
        if (!createdConversationId) throw new Error("Cannot run Test 8 without createdConversationId");
        await apiClient.deleteConversation(createdConversationId);
        console.log(`   [PASS] Attempted deletion of conversation ID: ${createdConversationId}`);
        ...
        try {
            ...
            console.error('   [FAIL] Fetching deleted conversation should have failed, but did not.');
        } catch (error) {
            if (error instanceof ApiError && error.status === 404) {
                console.log('   [PASS] Verified conversation deletion (received 404 as expected).');
            } else {
                 console.warn('   [WARN] Verification of deletion failed with unexpected error:', error);
            }
        }
...
const expect = (actual: any) => ({
    toBe: (expected: any) => {
        if (actual !== expected) {
            throw new Error(`Assertion Failed: Expected ${JSON.stringify(expected)}, but received ${JSON.stringify(actual)}`);
        }
    },
    toBeDefined: () => {

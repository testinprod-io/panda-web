...
const LoadingIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="30" height="14" fill="currentColor" viewBox="0 0 120 30">
    ...
    <circle cx="60" cy="15" r="9" fill="var(--primary, red)" fillOpacity=".3">
...
export function Loading(props: { noLogo?: boolean }) {
  return (
    <div className={clsx("no-dark", styles["loading-content"])}>
      {!props.noLogo && <BotIcon className="w-8 h-8" />}
      <LoadingIcon />
    </div>
  );
}
...
const loadAsyncGoogleFont = () => {
  ...
  const googleFontUrl =
    false ? remoteFontUrl : proxyFontUrl;
...
function Screen() {
  ...
  const shouldTightBorder =
    false || (config.tightBorder && !isMobileScreen);
  ...
  return (
    <div
      className={clsx(styles.container, {
        ...
        [styles["rtl-screen"]]: false, //getLang() === "ar",
      })}
    >
      {renderContent()}
    </div>

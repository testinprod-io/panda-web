...
const DEFAULT_CHAT_INTERACTION_STATE = {
  onSendMessageHandler: null as
    | ((sessionState: SessionState) => Promise<void>)
    | null,
  hitBottom: true,
    ...
    function mergeMessages(
      ...
      serverMessages.forEach((msg) => {
        messageMap.set(msg.id, {
          ...
          isError: false,
          streaming: false,
        }); // Assume server messages are final
      });
      ...
      localMessages.forEach((localMsg) => {
        ...
        if (!existing) {
          ...
          if (localMsg.syncState !== "synced") {
            // Don't add local 'synced' messages if not on server (avoids duplicates if server data changed)
          ...
          if (
            localMsg.syncState === "pending_create" ||
            localMsg.syncState === "error" ||
            localMsg.streaming
          ) {

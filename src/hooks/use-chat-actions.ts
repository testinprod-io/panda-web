...
export function useChatActions() {
  ...
  const loadSessionsFromServer = useCallback(
    async (options?: { cursor?: string | null; limit?: number }) => {
      if (!apiClient) {
        console.warn(
          "[ChatActions] API client not available, cannot load sessions.",
        );
      ...
      console.log(
        `[ChatActions] Loading sessions from server... Cursor: ${effectiveCursor}, Limit: ${effectiveLimit}`,
      );
      try {
        ...
        const response = await ChatApiService.fetchConversations(
          apiClient,
          apiParams,
        );

        console.log(
          `[ChatActions] Raw API response from fetchConversations: Received ${response.data.length} items. Pagination:`,
          response.pagination,
        );
        ...
        const fetchedAppSessions: ChatSession[] = serverConvos.map(
          (serverConv) => {
        ...
        console.log(
          `[ChatActions] Mapped ${fetchedAppSessions.length} sessions from server. Has more: ${pagination.has_more}, Limit: ${effectiveLimit}`,
        );
        ...
        console.error(
          "[ChatActions] Failed to load sessions from server:",
          error,
        );
        return { sessions: [], nextCursor: null, hasMore: false };
      }
  ...
  const newSession = useCallback(
    async (
      ...
      if (!apiClient) {
        console.warn(
          "[ChatActions] API client not available, cannot create new session on server.",
        );
        return undefined;
      }
  ...
  const loadMessagesForSession = useCallback(
    async (
      ...
      if (!apiClient) {
        console.warn(
          "[ChatActions] API client not available, cannot load messages.",
        );
      ...
      if (!session) {
        console.warn(
          `[ChatActions] Session ${id} not found in store. Skipping message load.`,
        );
      ...
      try {
        ...
        const pagination = response.pagination;
        console.log(
          `[ChatActions] Received ${serverApiMessages.length} messages for ${id}. Has more: ${pagination.has_more}`,
        );
        ...
        console.error(
          `[ChatActions] Failed loading messages for ${id}:`,
          error,
        );
  ...
  const selectSession = useCallback(
    (sessionId: UUID) => {
      ...
      if (indexToSelect === -1) {
        console.warn(
          `[ChatActions] Session with ID ${sessionId} not found in store. Cannot select.`,
        );
        return;
      }
      ...
      if (indexToSelect < 0 || indexToSelect >= sessions.length) {
        console.warn(
          `[ChatActions] Invalid session index derived: ${indexToSelect} for ID ${sessionId}`,
        );
        return;
      }
  ...
  const deleteSession = useCallback(
    async (sessionId: UUID) => {
      if (!apiClient) {
        console.warn(
          "[ChatActions] API client not available, cannot delete server session.",
        );
      ...
      if (indexToDelete === -1) {
        console.error(
          `[ChatActions] Attempted to delete non-existent session with ID ${sessionId}`,
        );
        return;
      }
      ...
      console.log(
        `[ChatActions] Deleting session ${indexToDelete} (ID: ${sessionId})`,
      );
      ...
      if (
        apiClient &&
        sessionToDelete.syncState !== "pending_create" &&
        sessionToDelete.syncState !== "local"
      ) {
        try {
          ...
          console.error(
            `[ChatActions] Failed to delete server session ${sessionId}:`,
            error,
          );
        ...
        console.log(
          `[ChatActions] Session ${sessionId} was local-only or pending creation, no server deletion needed.`,
        );
      }
    },
    [apiClient, store],
  ...
  const saveMessageToServer = useCallback(
    async (conversationId: UUID, message: ChatMessage) => {
      if (!apiClient) {
        console.warn(
          "[ChatActions] API client not available, cannot save message.",
        );
        return;
      }
      ...
      if (!localMessageId) {
        console.error(
          "[saveMessageToServer Action] Message missing local ID, cannot save.",
          message,
        );
        return;
      }
      ...
      const createRequest: MessageCreateRequest = {
        ...
        reasoning_time: message.reasoningTime
          ? message.reasoningTime.toString()
          : undefined,
        files: message.files,
        custom_data: {
      ...
      try {
        const savedMessage = await ChatApiService.createMessage(
          apiClient,
          conversationId,
          createRequest,
        );
        console.log(
          `[ChatActions] Message ${localMessageId} saved successfully (Server ID: ${savedMessage.message_id} TS: ${savedMessage.timestamp} ${savedMessage.sender_type})`,
        );
        message.syncState = MessageSyncState.SYNCED;
      } catch (error: any) {
        console.error(
          `[ChatActions] Failed saving message ${localMessageId}:`,
          error,
        );
      }
    },
  ...
  const generateSessionTitle = useCallback(
    async (
      ...
      if (!apiClient) {
        console.warn(
          "[ChatActions] API client not available, cannot generate title.",
        );
        return;
      }
      ...
      if (!session) {
        console.warn(
          `[Title Generation Action] Session not found (using getState): ${sessionId}.`,
        );
        return;
      }

      console.log(
        `[Title Generation Action] Starting for session: ${sessionId}`,
      );
      ...
      try {
        console.log(
          `[Title Generation Action] Calling LLM with config:`,
          titleGenConfig,
        );
        const generatedTitle = await ChatApiService.callLlmGenerateTitle(
          apiClient,
          prompt,
          titleGenConfig,
        );
        ...
        console.log(
          `[Title Generation Action] LLM generated title: "${generatedTitle}"`,
        );

        // Update local state only if the title is new and not the default
        if (
          generatedTitle !== DEFAULT_TOPIC &&
          generatedTitle !== session.topic
        ) {
          store.updateTargetSession({ id: sessionId }, (sess) => {
            sess.topic = encryptedTitle; // Update local topic
            sess.visibleTopic = generatedTitle;
            console.log(
              `[Title Generation Action] Updated local topic to: "${generatedTitle}"`,
            );
          });

          // Update server state if conversation exists
          console.log(
            `[Title Generation Action] Attempting to update server title for ConvID: ${session.id}`,
          );
  ...
  const updateConversation = useCallback(
    async (id: UUID, conversationData: ConversationUpdateRequest) => {
      if (!apiClient) {
        console.warn(
          "[ChatActions] API client not available, cannot update conversation.",
        );
        return;
      }
      ...
      try {
        const updatedConv = await ChatApiService.updateConversation(
          apiClient,
          session.id,
          conversationData,
        );
        console.log(
          `[ChatActions] Server title updated successfully to: "${updatedConv.title}"`,
        );
        store.updateTargetSession({ id: id }, (sess) => {
          sess.topic = updatedConv.title || sess.topic;
          sess.visibleTopic = EncryptionService.decrypt(
            updatedConv.title || sess.topic,
          );
          sess.lastUpdate = new Date(updatedConv.updated_at).getTime();
        });
      } catch (error) {
        console.error(
          `[ChatActions] Failed to update server title for ConvID ${id}:`,
          error,
        );
      }
    },
  ...
  const loadSummariesForSession = useCallback(
    async (
      sessionId: UUID,
    ): Promise<{
      ...
      if (!apiClient) {
        console.warn(
          "[ChatActions] API client not available, cannot load summaries.",
        );
        return { summaries: [], lastSummarizedMessageId: null };
      }

      console.log(
        `[ChatActions] Loading summaries for session ${sessionId}...`,
      );
      try {
        const fetchedSummaries = await apiClient.app.getSummaries(sessionId);
        console.log(
          `[ChatActions] Received ${fetchedSummaries.length} summaries for session ${sessionId}.`,
        );

        const sortedSummaries = [...fetchedSummaries].sort(
          (a, b) =>
        ...
        console.error(
          `[ChatActions] Failed to load summaries for session ${sessionId}:`,
          error,
        );
        return { summaries: [], lastSummarizedMessageId: null };
      }
  ...
  const summarizeAndStoreMessages = useCallback(
    async (
      sessionId: UUID,
      ...
      if (!apiClient || !apiClient.llm || !apiClient.app) {
        console.warn(
          "[ChatActions] API client, LLM service, or App service not available, cannot summarize.",
        );
        return undefined;
      }
      ...
      console.log(
        `[ChatActions] summarizeAndStoreMessages: Starting for session ${sessionId} with ${messagesToSummarize.length} messages.`,
      );
      ...
      try {
        ...
        console.log(
          `[ChatActions] LLM summary received. Length: ${summaryText.length}`,
        );
        ...
        const appSummaryResponse = await apiClient.app.createSummary(
          sessionId,
          summaryCreateRequest,
        );
        ...
        console.log(
          `[ChatActions] Summary stored on server. Summary ID: ${newSummary.summary_id}`,
        );
        return newSummary;
      } catch (error) {
        console.error(
          `[ChatActions] Failed to summarize and store messages for session ${sessionId}:`,
          error,
        );
        return undefined;
      } finally {
        ...
        console.log(
          `[ChatActions] summarizeAndStoreMessages: Process finished for session ${sessionId}.`,
        );
      }
    },
    [apiClient],

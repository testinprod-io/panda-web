...
class VaultService {
  ...
  private async handleSetPassword(request: any): Promise<any> {
    try {
      ...
      console.log(
        "[Vault] Password validation successful, proceeding to set password"
      );
  ...
  private async handleCreateUserPassword(request: any): Promise<any> {
    try {
      ...
      console.log(
        "[Vault] User password created, ID encrypted, and stored on server"
      );
      this.resetIdleTimer();
      ...
      return {
        id: request.id,
        error:
          error instanceof Error ? error.message : "password creation failed",
      };
    }
  }
  ...
  private async handleUpdateKey(request: any): Promise<any> {
    try {
      ...
      // Decrypt password
      console.log(
        "[Vault] Decrypting password with old key",
        request.encryptedPassword,
        "oldKey",
        oldKey
      );
      const passwordPlain = await decryptPassword(
        request.encryptedPassword,
        oldKey
      );
    ...
    try {
      ...
      if (request.encryptedPassword) {
        ...
        try {
          ...
          if (!oldKey) {
            console.log(
              "[Vault] No oldKey available, requiring password input"
            );
            return {
              id: request.id,
              ok: true,
          ...
          console.error(
            "[Vault] Failed to decrypt stored password:",
            decryptError
          );
          console.log(
            "[Vault] Stored password may be corrupted or from old format, requiring fresh password input"
          );
          return {
            id: request.id,
            ok: true,
        ...
        try {
          ...
          console.error(
            "[Vault] Failed to encrypt provided password:",
            encryptError
          );
        ...
        // Need password input
        console.log(
          "[Vault] No password or encrypted password provided, requiring input"
        );
        return {
          id: request.id,
          ok: true,
      ...
      try {
        ...
        console.error(
          "[Vault] Failed to import password as CryptoKey:",
          importError
        );
      ...
      console.log(
        "[Vault] Bootstrap successful, password validated and stored"
      );
      this.resetIdleTimer();
  ...
  private async handleDerive(request: any): Promise<any> {
    try {
      ...
      console.log(
        "[Vault] Password CryptoKey is ready for encryption operations"
      );
  ...
  private async handleEncrypt(request: any): Promise<any> {
    ...
    try {
      ...
      const encrypted = await this.encryptWithPasswordKey(
        request.plain,
        this.passwordKey
      );
  ...
  private async handleDecrypt(request: any): Promise<any> {
    ...
    try {
      ...
      const plain = await this.decryptWithPasswordKey(
        request.encrypted,
        this.passwordKey
      );
  ...
  private async handleClearKeys(request: any): Promise<any> {
    try {
      ...
      return {
        id: request.id,
        error: error instanceof Error ? error.message : "clearKeys failed",
      };
    }
  }
  ...
  private async handleEncryptFile(request: any): Promise<any> {
    ...
    try {
      ...
      const encryptedData = await this.encryptFileWithPasswordKey(
        request.fileData,
        this.passwordKey
      );
  ...
  private async handleDecryptFile(request: any): Promise<any> {
    ...
    try {
      ...
      const decryptedData = await this.decryptFileWithPasswordKey(
  ...
  private async encryptWithPasswordKey(
    plaintext: string,
  ...
  private async decryptWithPasswordKey(
  ...
  private async encryptFileWithPasswordKey(
  ...
  private async decryptFileWithPasswordKey(
  ...
  private async fetchServerKeys(): Promise<{
    oldKey: string | null;
    newKey: string;
  }> {
    try {
  ...
  private async createEncryptedIdOnServer(encryptedId: string): Promise<void> {
    try {
      ...
      console.log(
        "[Vault] Creating encrypted ID on server via BFF:",
        createEncryptedIdUrl
      );
  ...
  private async validatePassword(password: string): Promise<boolean> {
    if (!this.encryptedId || !this.expectedUserId) {
      console.log(
        "[Vault] No encryptedId or expectedUserId available for validation"
      );
      return false;
    }

    return this.validatePasswordAgainstEncryptedId(
      password,
      this.encryptedId,
      this.expectedUserId
    );
  ...
  private async validatePasswordAgainstEncryptedId(
    password: string,
    encryptedId: string,
    ...
    try {
      ...
      const expectedEncryptedId = this.generateEncryptedId(
        password,
        expectedUserId
      );
